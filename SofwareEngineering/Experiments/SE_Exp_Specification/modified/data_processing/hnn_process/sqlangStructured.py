    ...
    def identifyLiterals(self, tokenList):
        ...
        for tok in tokenList.tokens:
            if isinstance(tok, sqlparse.sql.TokenList):
                ...
                tok.ttype = WILDCARD
            elif tok.ttype in blankTokens or isinstance(tok, list):
                tok.ttype = COLUMN
...
def processNlLine(line):
    ...
    space = re.compile(r"\([^\(|^\)]+\)")  # 去除括号里内容, 后缀匹配
    line = re.sub(space, '', line)
    line = line.strip()  # 去除末尾.和空格
    return line
...
def filterAllInvachar(line):
    ...
    line = re.sub('-+', '-', line)  # 去除非常用符号；防止解析有误
...
def sqlangQueryParse(line):
    ...
    for i in range(0, len(word_list)):  # 分完词后,再去掉 括号
...
def sqlangContextParse(line):
    line = filterPartInvachar(line)
    line = processNlLine(line)
    word_list = processSentWord(line)
    word_list = [x.strip() for x in word_list if x.strip() != '']  # 列表里包含 '' 或 ' '
    return word_list  # 解析可能为空


if __name__ == '__main__':
    print(sqlangCodeParse(
