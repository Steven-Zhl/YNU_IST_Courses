...
import inflection  # 骆驼命名法
from nltk import pos_tag  # 词性还原
from nltk import wordpunct_tokenize
from nltk.corpus import wordnet  # 词干提取
...
def getVars(ast_root):
    return sorted({node.id for node in ast.walk(ast_root)
...
def getVarsHeuristics(code):
    ...
    for line in code_lines[end:]:
        ...
        try:
            ...
            pattern_var_equal_matched = re.match(PATTERN_VAR_EQUAL, line)  # matching PATTERN_VAR_EQUAL
            ...
            pattern_var_for_matched = re.search(PATTERN_VAR_FOR, line)  # matching PATTERN_VAR_FOR
...
def pythonParser(code):
    ...
    bool_first_success = firstTrial(code)
...
def processSentWord(line):
    line = re.findall(r"[\w]+|[^\s\w]", line)  # 找单词
    line = ' '.join(line)
    decimal = re.compile(r"\d+(\.\d+)+")  # 替换小数
    line = re.sub(decimal, 'TAGINT', line)
    string = re.compile(r'\"[^\"]+\"')  # 替换字符串
    line = re.sub(string, 'TAGSTR', line)
    decimal = re.compile(r"0[xX][A-Fa-f0-9]+")  # 替换十六进制
    line = re.sub(decimal, 'TAGINT', line)
    number = re.compile(r"\s?\d+\s?")  # 替换数字 56
    line = re.sub(number, ' TAGINT ', line)
    other = re.compile(r"(?<![A-Z|a-z|_|])\d+[A-Za-z]+")  # 替换字符 6c60b8e1, 后缀匹配
...
def pythonCodeParse(line):  # 主函数：代码的tokens
    ...
    '''
    try:
        typedCode, failed_var, failed_token = pythonParser(line)  # 骆驼命名转下划线
        typedCode = inflection.underscore(' '.join(typedCode)).split(' ')

        cut_tokens = [re.sub("\s+", " ", x.strip()) for x in typedCode]  # 全部小写化
        token_list = [x.lower() for x in cut_tokens]  # 列表里包含 '' 和' '
        token_list = [x.strip() for x in token_list if x.strip() != '']
        return token_list  # 存在为空的情况，词向量要进行判断
    except:
        return '-1000'
...
def pythonQueryParse(line):
    ...
    for i in range(0, len(word_list)):
        if re.findall('[()]', word_list[i]):
            word_list[i] = ''
    # 列表里包含 '' 或 ' '
    word_list = [x.strip() for x in word_list if x.strip() != '']  # 解析可能为空
    return word_list


def pythonContextParse(line):
    line = filterPartInvachar(line)  # 在这一步的时候驼峰命名被转换成了下划线
    line = processNlLine(line)
    print(line)
    word_list = processSentWord(line)  # 列表里包含 '' 或 ' '
    word_list = [x.strip() for x in word_list if x.strip() != '']  # 解析可能为空
    return word_list
...
if __name__ == '__main__':
    print(pythonQueryParse("change row_height and column_width in libreoffice calc use python tagint"))
    print(pythonQueryParse('What is the standard way to add N seconds to datetime.time in Python?'))
    print(pythonQueryParse("Convert INT to VARCHAR SQL 11?"))
    print(pythonQueryParse(
        'python construct a dictionary {0: [0, 0, 0], 1: [0, 0, 1], 2: [0, 0, 2], 3: [0, 0, 3], ...,999: [9, 9, 9]}'))

    print(pythonContextParse(
    ...
    print(pythonContextParse('how do i display records (containing specific) information in sql() 11?'))
    print(pythonContextParse('Convert INT to VARCHAR SQL 11?'))

    print(pythonCodeParse(
        'if(dr.HasRows)\n{\n // ....\n}\nelse\n{\n MessageBox.Show("ReservationAnd Number Does Not Exist","Error", MessageBoxButtons.OK, MessageBoxIcon.Asterisk);\n}'))
    print(pythonCodeParse('root -> 0.0 \n while root_ * root < n: \n root = root + 1 \n print(root * root)'))
    print(pythonCodeParse('root = 0.0 \n while root * root < n: \n print(root * root) \n root = root + 1'))
    print(pythonCodeParse('n = 1 \n while n <= 100: \n n = n + 1 \n if n > 10: \n  break print(n)'))
    print(pythonCodeParse(
    ...
    print(pythonCodeParse("d = {'x': 1, 'y': 2, 'z': 3} \n for key in d: \n  print (key, 'corresponds to', d[key])"))
    print(pythonCodeParse(
        '  #       page  hour  count\n # 0     3727441     1   2003\n # 1     3727441     2    654\n # 2     3727441     3   5434\n # 3     3727458     1    326\n # 4     3727458     2   2348\n # 5     3727458     3   4040\n # 6   3727458_1     4    374\n # 7   3727458_1     5   2917\n # 8   3727458_1     6   3937\n # 9     3735634     1   1957\n # 10    3735634     2   2398\n # 11    3735634     3   2812\n # 12    3768433     1    499\n # 13    3768433     2   4924\n # 14    3768433     3   5460\n # 15  3768433_1     4   1710\n # 16  3768433_1     5   3877\n # 17  3768433_1     6   1912\n # 18  3768433_2     7   1367\n # 19  3768433_2     8   1626\n # 20  3768433_2     9   4750\n'))
