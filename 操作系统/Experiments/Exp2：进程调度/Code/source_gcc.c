...
typedef struct PCB {
    int PID;                     // 进程ID
    int State;                  // 状态（就绪|结束），1表示就绪，0表示结束
    int Total_time;              // 该进程所需的总时间（要求运行时间）
    int CPU_time;                // 该进程已经在CPU中运行的时间（已运行时间）
    struct PCB *Next_PCB; // 指示下一进程
    int Priority;            // 【仅动态优先级调度使用】优先级
    int Wait_time;           // 【仅最高响应比调度使用】等待时间
} *Proc;
...
int Init_PCB_DHPF() {
    ...
    for (i = Proc_num; i > 1; i--) {
        ...
        temp->State = 1;
        temp->CPU_time = 0;
        if (temp->Priority >= head->Priority) { // 进程temp优先级最高，将其放在队头
            temp->Next_PCB = head;
            head = temp;
        } else {
            if (temp->Priority <= tail->Priority) { // 进程temp优先级最低，将其放在队尾
                tail->Next_PCB = temp;
                tail = temp;
            } else { // temp优先级在队列中，则循环找到应该插入队列的地方
...
void Display_HRRF() {
    ...
    for (i = 0; i < Proc_num; i++) {
        printf(" | %-10d\t%-19d\t%-23d\t%-21.4f |\n", ptr->PID, ptr->CPU_time, ptr->Total_time,
               1 + ptr->Wait_time / (float) (ptr->Total_time));
        ptr = ptr->Next_PCB;
    }
}
...
void Sched_FCFS() {
    ...
    while (p->Total_time > p->CPU_time) {
        ...
        p->CPU_time++;
        Display_FCFS();
        if (p->Total_time == p->CPU_time) {                     // 进程运行时间已达到要求的时间，表示该进程已执行完毕
            p->State = 0; // 置结束标志
            Proc_num--;
...
void Sched_DHPF() { // 由于在初始化的时候就按照优先级降序进行过排列了，所以在顺序执行的时候优先级降序仍然维持。
    int round = 1, i;
    Proc temp = tail;
    Proc p = head;
    while (p->Total_time > p->CPU_time) {
